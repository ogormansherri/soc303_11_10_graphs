...
```{r lreg1a, results='markup', eval=TRUE, echo=TRUE}
library(ggplot2)
social.alienation=as.numeric(c(25,30,35,40,40))
religiosity=as.numeric(c(10,9,8,8,7))
religion=data.frame(cbind(social.alienation, religiosity))
religion
r=qplot(social.alienation, religiosity)
r+stat_smooth(method="lm")
```
...
```{r social_reg, results='markup', eval=TRUE, echo=TRUE}
...
religion=data.frame(cbind(social.alienation, religiosity))
religion
s=qplot(social.alienation, religiosity)
...
## Examples: Regression using the Diamonds

http://www.diamondse.info/ collected in 2008. Prices of 50,000 round cut diamonds
...
Let's look at this a little differently with a histogram:

```{r diamond_hist, results='markup', eval=TRUE, echo=TRUE}
with(diamonds, hist(price))
```



```{r d1, results='markup', eval=TRUE, echo=TRUE}
data(diamonds)
plot(diamonds$carat, diamonds$price,
     xlab = "Mass (carats)",
     ylab = "Price (SIN $)",
     bg = "lightblue",
     col = "red", cex = 1.1, pch = 21,frame = FALSE)
